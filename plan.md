...
当前候选已删除旧backend/frontend及构建入口，run.sh仅启动新服务；旧部署仍在独立
checkout及源码归档中保留。生产停写审计发现两个不属于Agent部署的容器可写整个/raid，
须确认归属并消除这项写入风险后才能完成实际迁移切换、观察和旧部署退役。
生产仍运行旧服务，不能将候选源码清理或私有迁移演练称为完整部署完成。
