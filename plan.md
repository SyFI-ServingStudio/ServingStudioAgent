...
已删除旧backend/frontend及构建入口，run.sh仅启动新服务；清理后627项回归通过。
生产于2026-09-11完成停写、独立状态备份、7个数据库迁移和新服务启动。原状态与备份
清单一致，4个原会话的10条消息及原有引用/活动记录保持，5个turn可重放。
实际部署的Claude/Codex各一次隔离调用、桌面/手机浏览器及旧新回调鉴权均通过。
用户明确接受两个广域挂载容器保持运行；审计持续核对其完整ID和挂载，未停止这两个容器。
旧部署进程及所属容器已停止，源码/原状态/备份作为恢复资料保留。外部实验repo/logs
仍在原位，不属于Agent状态备份覆盖。新用户写入开始后不得直接覆盖回旧快照。
详见doc/cutover.md的Completed Cutover；后续阶段条目保留实施过程中的历史记录。
