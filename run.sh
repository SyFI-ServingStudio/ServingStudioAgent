#!/usr/bin/env bash
# Launch the MLSim chat UI. Backend shells out to `codex`; it does not import MLSim,
# so this runs in its own light uv venv (fastapi + uvicorn only).
set -euo pipefail
cd "$(dirname "$0")"

export CODEX_DOCKER_IMAGE="${CODEX_DOCKER_IMAGE:-mlsim-ui-codex-runner:latest}"
