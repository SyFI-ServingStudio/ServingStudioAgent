...
export CODEX_RUNNER_IMAGE_VERSION="${CODEX_RUNNER_IMAGE_VERSION:-prebuilt-codex-runner-v8}"
...
current_build_sha="$({
  ...
  git -C ../main diff --binary HEAD -- Cargo.toml Cargo.lock .cargo simulator analyzer
} | sha256sum | awk '{print $1}')"
export CODEX_MAIN_BUILD_SHA="$current_build_sha"
...
if [ "${CODEX_SKIP_IMAGE_BUILD:-0}" != "1" ]; then
  ...
  image_build_sha="$(docker image inspect -f '{{ index .Config.Labels "org.vibesim.ui.main-build-sha" }}' "$CODEX_DOCKER_IMAGE" 2>/dev/null || true)"
  ...
  if [ "${CODEX_FORCE_IMAGE_BUILD:-0}" = "1" ] \
    ...
    || [ "$image_lock_sha" != "$current_lock_sha" ] \
    || [ "$image_build_sha" != "$current_build_sha" ]; then
