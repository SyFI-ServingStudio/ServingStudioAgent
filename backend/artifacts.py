...
def workspace_tree(workspace_id: str) -> WorkspaceTree:
    ...
    if logs is not None and logs.is_dir() and logs != repo and repo not in logs.parents:
        roots.append(logs)
...
def guard_artifact(tree: WorkspaceTree, rel_or_abs: str) -> Path:
    ...
    if requested == Path("/workspace") or Path("/workspace") in requested.parents:
        requested = tree.repo / requested.relative_to("/workspace")
    elif not requested.is_absolute():
        requested = tree.repo / requested
    try:
...
def preview_kind(path: Path) -> str:
    ...
    try:
        ...
        # A multi-byte character may straddle the sniff boundary; only the tail
...
def artifact_meta(workspace_id: str, rel_path: str) -> dict[str, Any]:
    """Describe one path (file or directory) without reading its content."""
    tree = workspace_tree(workspace_id)
...
def list_artifacts(
    ...
    *,
    ...
    walk_root = next(root for root in tree.roots if start == root or root in start.parents)
            ...
            entries.append(
                {
                    ...
                    "mtime": int(stat.st_mtime),
                    "is_dir": is_dir,
                    "preview_kind": "directory" if is_dir else preview_kind(child),
                }
            )
...
def resolve_artifact(workspace_id: str, rel_path: str) -> Path:
    ...
    resolved = guard_artifact(workspace_tree(workspace_id), rel_path)
    if not resolved.is_file():
        raise FileNotFoundError(str(rel_path))
    return resolved


def resolve_preview(workspace_id: str, rel_path: str) -> tuple[Path, str]:
    """Resolve one file for untokened browser preview. Returns (path, kind)."""
    resolved = guard_preview(workspace_tree(workspace_id), rel_path)
    if not resolved.is_file():
        raise FileNotFoundError(str(rel_path))
